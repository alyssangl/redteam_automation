...
def _run_with_timeout(fn, args=(), kwargs=None, timeout=20, on_timeout="(timed out)"):
    ...
    daemon, so a wedged worker abandoned via shutdown(wait=False) kept the cell
    subprocess alive PAST 'EXECUTION COMPLETE' to the 40-min cell cap. A daemon
    thread is abandoned cleanly and never blocks interpreter exit."""
    kwargs = kwargs or {}
    box: dict = {}
