"""
Live test driver for examples/flaw_impact_graph.py.

UnrealIRCd 3.2.8.1 backdoor (CVE-2010-2075) chain against Metasploitable 3
on port 6667. Lands a non-root command shell, then deliberately attempts a
proof-of-compromise write to a ROOT-ONLY path. Expected: the impact stage /
replanner detects the Permission-denied failure and adapts to a writable
/tmp path.
"""

from __future__ import annotations
import os

import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

from core_agents.orchestrator import run_graph
from examples.flaw_impact_graph import build_flaw_impact_graph

TARGET_IP = "192.168.34.7"
ATTACKER_IP = os.getenv("LHOST", "192.168.34.1")

if __name__ == "__main__":
    graph = build_flaw_impact_graph(target_ip=TARGET_IP, attacker_ip=ATTACKER_IP)
    print(f"Loaded graph: {graph.name}  ({len(graph.nodes)} nodes, {len(graph.edges)} edges)")
    print(f"Running with explore=True, use_judge=True against {TARGET_IP}...")
    run_graph(graph, explore=True, use_judge=True)
