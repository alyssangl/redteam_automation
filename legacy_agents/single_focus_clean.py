...
def llm_node(state: AgentState):
    ...
    sys_msg = SystemMessage(
        content=(
            ...
            "   - **Contextual Filling**: If a module requires an option (e.g., SITEPATH, TARGETURI) and the RAG example is ambiguous or missing it, USE YOUR GENERAL KNOWLEDGE to select standard defaults (e.g., '/var/www/html' for web roots).\n"
...
def critic_node(state: AgentState):
    """The Critic: Reviews the Worker's execution by checking HISTORY."""
    messages = state['messages']
    last_message = messages[-1]

    # 1. SKIP CRITIQUE if the agent just requested a tool call (ReAct loop)
    if last_message.tool_calls:
        # FIXED: Return explicit empty list to match state definition, avoids NoneType error
        return {"messages": []}

    # 2. CHECK HISTORY: Did we actually run a tool in the last turn?
    # We look at the message BEFORE the last one. It should be a ToolMessage.
    ...
    if len(messages) >= 2:
        ...
        if isinstance(previous_msg, ToolMessage):
            ...
            tool_outputs = previous_msg.content

    # 3. Construct the Critique Prompt with CONTEXT
    critic_model = ChatOpenAI(model=MODEL_NAME, temperature=0)

    # We feed the Critic the Tool Output so it knows what happened
    context_block = f"""
    ...
    **OUTPUT FORMAT:**
    ...
    # FIXED: Return explicit empty list instead of {} to prevent iteration errors
    return {"messages": []}
